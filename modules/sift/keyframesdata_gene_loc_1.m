...
function [ty1, ty2] = keyframesdata_gene_loc_1(tr_num, val_num, psz, half_range, saveDir, seed, data_root)
...
files = dir(data_root)
for idx = 1:min(tr_num, length(files))
    if ~files(idx).isdir
        fn = fullfile(data_root, files(idx).name);
        tmp = disp_read(fn);
...
for idx = 1:min(tr_num, length(files))
    if ~files(idx).isdir
        ...
        for loc = 1:length(r)
            l_center_x = c(loc);
            l_center_y = r(loc);
            r_center_x = round(c(loc) - tmp(r(loc), c(loc))); % its '-' not '+'
...
for idx = 1:val_num
    ...
    if ~files(f_idx).isdir
        fn = fullfile(data_root, files(f_idx).name);
        tmp = disp_read(fn);
        num_loc(f_idx) = sum(tmp(:)~=-1);
        num_pixel(f_idx) = numel(tmp);
    end
end